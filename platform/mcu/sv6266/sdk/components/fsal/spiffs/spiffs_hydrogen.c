...
s32_t prvSPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
  ...
  printf("page index byte len:         %"PRIu32"\n", SPIFFS_CFG_LOG_PAGE_SZ(fs));
  printf("object lookup pages:         %lu\n", SPIFFS_OBJ_LOOKUP_PAGES(fs));
  printf("page pages per block:        %"PRIu32"\n", SPIFFS_PAGES_PER_BLOCK(fs));
  printf("page header length:          %lu\n", sizeof(spiffs_page_header));
  printf("object header index entries: %lu\n", SPIFFS_OBJ_HDR_IX_LEN(fs));
  printf("object index entries:        %lu\n", SPIFFS_OBJ_IX_LEN(fs));
  printf("available file descriptors:  %"PRIu32"\n", fs->fd_count);
...
s32_t prvSPIFFS_rename(spiffs *fs, const char *old_path, const char *new_path) {
#if SPIFFS_READ_ONLY
...
#if SPIFFS_OBJ_META_LEN
s32_t prvSPIFFS_update_meta(spiffs *fs, const char *name, const void *meta) {
#if SPIFFS_READ_ONLY
...
s32_t prvSPIFFS_fupdate_meta(spiffs *fs, spiffs_file fh, const void *meta) {
#if SPIFFS_READ_ONLY
